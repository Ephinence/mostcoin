...
uint256 hashGenesisBlock("0x00000c18d35ea6cb50624f751c499a08fbbb124c53f58feddf3c93afff86d0e7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MostCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MostCoin Signed Message:\n";
...
{
    // MostCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
		return 15000000000 * COIN;

    nSubsidy >>= (nHeight / 166666667);
...
{
    // MostCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "most genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1497928633;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1010679;
...
unsigned char pchMessageStart[4] = { 0xf5, 0x22, 0xb9, 0xde }; // MostCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MostCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MostCoinMiner : ProcessBlock, block not accepted");
    }
